...
def main() -> int:
    ...
    a("")
    a(f"**Site:** {site}  ")
    # Only printed when the target actually came from OpenAQ. The cross-city
    # record does not, and stating IDs there would misattribute its provenance.
    location_ids = cfg.get("data.openaq.location_ids")
    if location_ids:
        a(f"**OpenAQ location IDs:** {location_ids}  ")
