...
fragmentation of the monitoring record, rather than the model class, decides which method
wins. Nine years of hourly observations from Dhaka (7.44 usable years, 82.3 per cent hourly
coverage) are fused with reanalysis meteorology into 102 predictors, and compact sequence
models under a 100{,}000 parameter budget are compared against tree ensembles, linear models
and the classical baselines air quality work often omits, at horizons from 1 to 24 hours. At
24 hours the validation selected recurrent model (16{,}193 parameters) attains an RMSE of
57.70 \ugm{} against the best classical model at 58.50, with a Holm adjusted Diebold and
Mariano $p$ of 0.037; a 95 per cent model confidence set nonetheless retains five of nine
candidates, so the finding is about tiers and not architectures. A controlled gap injection
experiment then degrades a near complete record two ways that remove identical numbers of
observed hours and differ only in arrangement. The sequence tier loses 0.0394 skill to
arrangement alone (50 matched pairs, Holm $p<0.0001$) and is the only family whose loss is
significant on all three donor stations, although it is not separable from tree ensembles when
families are differenced within matched triples. Because a scored row needs unbroken history, a
gap costs the hours it removes plus the backward reach behind them. Shortening that reach from
192 to 48 hours removes 97 per cent of the penalty, and a closed form for usable rows fitted on
101 cells predicts 404 held out cells at $R^2=0.982$. Availability is therefore reported as a
first class metric, since the configured reach answers only 76.4 per cent of test hours and
shortening it costs no measurable accuracy.
\end{abstract}
...
ranking that moves between them is attributable to everything simultaneously. What is needed
is a counterfactual on a single record. This paper builds one. We assemble a leakage
controlled pipeline over nine years of hourly PM\textsubscript{2.5} from Dhaka fused with
reanalysis meteorology, benchmark three tiers of method at five horizons, and then hold the
record fixed while manipulating only the arrangement of its absences. Fig.~\ref{fig:arch}
shows the design. Our contributions are as follows.
...
unbroken history over a sterilisation radius; shortening it from 192 to 48 hours removes
...
\begin{figure*}[!t]
\centerline{\includegraphics[width=0.80\textwidth]{fig00_architecture.pdf}}
...
bearing decision, which is why both proposed linear models enter here as first class
competitors.
...
Four components are described in turn below. Two hourly records supply the target and its
drivers. A gap aware rule decides which rows may be scored at all, and it governs everything
after it. Three tiers of method are then compared under a selection rule that never consults
test error, and a controlled degradation makes fragmentation the manipulated variable.
...
heavily collinear input and handicapped the tier against competitors the same columns help.

\subsection{What counts as a scored row}
...
and tier two by cross validation score. Tier one is ranked on test error only because it has
no hyperparameters and therefore nothing to bias. The rule has a measurable cost, which we
...
count, so volume is held constant and no result is explained by there being less data. The
evaluation period is never degraded, so every cell is scored on identical rows. The optimizer
step budget, not the epoch budget, is equalised across cells, because fragmentation shrinks
the training set and a fixed epoch count would leave a fragmented cell undertrained rather
than damaged. Hyperparameters are frozen at the undegraded record's selections, as is each
family's representative model, so a difference between arms cannot absorb a change of model.
...
all, and that condition is rarely reported. We define forecast availability as the share of
evaluation hours a configuration can answer. Because a model that declines the hard hours
flatters itself on the hours it accepts, error on the served set is never compared across
configurations of differing availability. The two valid comparisons are the common subset
served by every arm and an all hours score, over a fixed universe, in which an unanswered hour
falls back to persistence.

\section{Results}

Results follow the order the argument requires rather than the order of their strength. The
benchmark comes first, because its limits motivate everything after it. The two city contrast
follows and fails to identify anything, the controlled experiment then supplies what that
contrast cannot, and the mechanism, the closed form and the decision rule close the section.

\begin{figure*}[!t]
\centerline{\includegraphics[width=0.78\textwidth]{fig11_gap_injection.pdf}}
...
\subsection{The benchmark, and what it can separate}
...
rejects equality with a Holm adjusted $p$ of 0.037. Intervals there are a moving block
bootstrap with 1{,}000 resamples over 24 hour blocks, taken in blocks because consecutive
hourly errors are correlated.
...
Any single horizon headline is therefore a selection, and we report the four horizons that do
not support ours next to the one that does.
...
candidates win at least once, yet the consequence is bounded, because the candidates validation
cannot separate are near ties. Selecting on validation rather than on test costs $+0.05$ \ugm{}
here. We therefore make claims about tiers, not about architectures.
...
Seasonal naive      & 73.76 & [67.77, 79.86] & $\;\;\;0.000$ & no \\
Persistence         & 73.76 & [67.77, 79.86] & $\;\;\;0.000$ & no \\
SARIMAX             & 81.36 & [74.83, 87.93] & $-0.103$ & no \\
\hline
\multicolumn{5}{l}{\footnotesize RMSE in \ugm. The GRU row is the validation selected seed, whose}\\
\multicolumn{5}{l}{\footnotesize predictions enter the paired tests; the five seed mean is $57.70\pm0.44$.}\\
\hline
\end{tabular}
\end{table}

\subsection{The observational contrast identifies nothing}
...
recommend a different method. The comparison is nevertheless uninformative about fragmentation,
in two ways. First, the comparison city cannot support a ranking at all, because its own
...
seasonal transition. We report this rather than set it aside, and treat it as the reason to
build a counterfactual.

\subsection{Arrangement costs skill at constant volume}
...
families. The
sequence tier loses 0.0394 skill to arrangement alone, and it is not alone in losing
something. The climatological and tree families also move, and only the linear family is
indistinguishable from zero, so the honest summary is that fragmentation is costly to most of
the methods considered and most costly to the one that reads windows.


\subsection{Replication, and the contrast that does not survive}
...
family is instructive here and is the reason more than one donor was run. On the first donor
alone its penalty collapses to $-0.096$ at the severest degradation, which read in isolation
would have argued that fragmentation degrades anything estimated from the record. It does not
survive the other two stations, so that collapse is a property of that station.
...
comparative claim and state the narrower one that survives, namely that the sequence tier's
penalty is the only one appearing reliably across records and the largest in mean.
...
\cline{2-4}\cline{5-8}\cline{9-10}
...
\hline
\multicolumn{10}{l}{\footnotesize Fifty matched pairs per family per donor; representatives are fixed on the undegraded record. Differencing families within matched triples does}\\
\multicolumn{10}{l}{\footnotesize not separate sequence from trees ($-0.0102$, $p=0.15$), so the comparative claim is withheld.}\\
\hline
\end{tabular}
\end{table*}

\subsection{The backward reach carries the effect}
...
the reach is set by configuration rather than inferred from a regression, this is a stronger
design than a regression based mediation and needs no ignorability assumption. The injection
is deterministic in arm, coverage and seed and runs before any feature is built, so the same
...
is the imputation policy rather than a fitted parameter. The practical consequence is that a
near complete record is not a well supervised one. Amplification, being usable hours destroyed
per hour missing, runs at 2.3 on the primary record and at 34.2, 14.0 and 10.4 on the three
near complete stations, because it follows the number of gaps and not their total length.

\begin{figure*}[!t]
\centerline{\includegraphics[width=0.76\textwidth]{fig14_missingness_amplification.pdf}}
...
\subsection{What the reach costs, and what shortening it buys}
...
Divided through by $U/R$ this becomes an elasticity, the only form of the price comparable
across records of different sizes. Table~\ref{tab:avail} gives it. At the configured radius the
primary record pays 0.372 against the donor's 0.139, so a reach one per cent deeper costs it
2.7 times as large a share of its supervision. The same table shows why coverage is the wrong
summary statistic. Hourly coverage runs 82.3 per cent on the primary record against 98.9 on the
...
against 73.76. It is a model beating persistence over a large block of ordinary hours. And the
free price sizes the opportunity without promising the payoff, since the primary record recovers
48 per cent of its training rows against the donor's 9 per cent and gains less skill.
Availability rather than rows orders the two correctly.
...
Several limits are worth stating rather than leaving to a referee. The injection experiment runs
at one horizon with one injected gap length distribution, and its three donors are stations of a
single archive covering one four year window and one weather regime. That makes it a station
robustness check, able to falsify a station specific effect but not evidence of generality
across cities. The comparative claim that the sequence tier suffers most does not survive an
intersection union test and is not made, and the mediation result is established only on the
record where the penalty was measured. The target series comes from a single monitor, so
results describe that site rather than Dhaka as a whole. Ridge is structurally disadvantaged by
a logarithmic target with raw scale history and should be read as a linear floor rather than a
tuned competitor. Finally, inverting that logarithmic scale returns something nearer a
conditional median, so every model here underpredicts the largest episodes. The bias is common
to all of them, so between model comparisons remain valid.
...
because here the configured 168 hour reach bought no accuracy and cost fourteen points of the
hours a forecast could be issued at all.
